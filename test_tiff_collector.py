...
@nose.tools.istest
def tiff_files_copied_from_subsubdirs_to_execution_dir():
    # 1. create a temporary directory for the test
    temp_dir_path = 'my_test_dir%d' % random.randint(1, 1000)
    ...
    subsubdir_path = os.path.join('01', 'sub01', 'img_005.tif')
    error_msg = "Expected %s to be copied"\
                " to execution directory as %s."\
                % (subsubdir_path, subsubdir_filename)
    nose.tools.ok_(subsubdir_filename in tifs_in_exec_dir, error_msg)
...
@nose.tools.istest
def tiff_files_not_copied_from_subsubsubdirs_to_execution_dir():
    # 1. create a temporary directory for the test
    temp_dir_path = 'my_test_dir%d' % random.randint(1, 1000)

    # 2. copy test data and tiff script to test directory
    shutil.copytree('test_data3', temp_dir_path)
    shutil.copy('subdir_tiff_collector.py', temp_dir_path)
    os.chdir(temp_dir_path)
    ...
    # 4. check that the tif files have been copied
    tifs_in_exec_dir = glob.glob('*.tif')
    subsubsubdir_filename = '01_sub01_subsub01_img_006.tif'
    subsubsubdir_path = os.path.join('01', 'sub01', 'subsub01', 'img_006.tif')
    error_msg = "Did not expect %s to be copied"\
                " to execution directory as %s."\
                % (subsubsubdir_path, subsubsubdir_filename)
    nose.tools.ok_(not subsubsubdir_filename in tifs_in_exec_dir, error_msg)
